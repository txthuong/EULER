...
# A_BX_EmbeddedSW_STAMode_0004                  Use command +SRWSTACFG and +SRWSTACON to configure automate connection to another BX module that play a role as AP
#
# Requirement
# 2 Euler modules
#  
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_STAMode_0004
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_STAMode_0004"
...
try:
    ...
    print "\nStep 10: Reset module B\n"
    SagSendAT(aux1_com, 'AT+RST\r')
    SagWaitnMatchResp(aux1_com, ['*\r\nREADY\r\n'], 5000)
    SagWaitnMatchResp(uart_com, ['*+SRWAPSTA: 1,"%s"\r\n' % aux1_mac_address_sta], 5000 )
    
    print "\nStep 11: Check module auto connect to AP\n"
    if SagWaitnMatchResp(aux1_com, ['*\r\n+SRWSTASTATUS: 1,"%s","%s",5,3\r\n' % (wifi_ssid, dut_mac_address)], 20000):
        SagWaitnMatchResp(aux1_com, ['\r\n+SRWSTAIP: "%s.2","%s","%s"\r\n' % (return_subnet(wifi_dhcp_gateway), wifi_dhcp_subnet_mask, wifi_dhcp_gateway)], 10000)
    else:
        raise Exception("---->Problem: Module cannot connect to Wi-Fi !!!")
