...
# A_BX_EmbeddedSW_AP_STA_0060                   Check that device configured as Wi-Fi STA have a static IP or get one over DHCP
#
# Requirement
# 2 Euler modules
#  
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_AP_STA_0060
# -----------------------------------------------------------------------------------

test_ID = "A_BX_EmbeddedSW_AP_STA_0060"
...
try:
    ...
    print "*****************************************************************************************************************"
    print "%s:Check that device configured as Wi-Fi STA have a static IP or get one over DHCP" % test_ID
    print "*****************************************************************************************************************"
    ...
    print "\nStep 8: Configure network configuration for STA\n"
    SagSendAT(aux1_com, 'AT+SRWSTANETCFG=0,"%s.6","%s","%s"\r'%(return_subnet(wifi_dhcp_gateway), wifi_dhcp_subnet_mask, wifi_dhcp_gateway))
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    ...
    if SagWaitnMatchResp(aux1_com, ['*+SRWSTASTATUS: 1,"%s","%s",*,*\r\n' % (wifi_ssid, dut_mac_address)], 20000):
        SagWaitnMatchResp(aux1_com, ['+SRWSTAIP: "%s.6","%s","%s"\r\n' % (return_subnet(wifi_dhcp_gateway), wifi_dhcp_subnet_mask, wifi_dhcp_gateway)], 10000)
    else:
        raise Exception("---->Problem: Module cannot connect to Wi-Fi !!!")
    SagWaitnMatchResp(uart_com, ['\r\n+SRWAPSTA: 1,"%s"\r\n' % aux1_mac_address_sta], 4000)
...
SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
SagWaitnMatchResp(aux1_com, ['+SRWSTASTATUS: 0,8\r\n'], 2000)
