...
# A_BX_EmbeddedSW_KUDPCLOSE_0002                Check UDP server operation can be closed by "+KUDPCLOSE"
#
# Requirement
# 1 Euler module
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KUDPCLOSE_0002
# -----------------------------------------------------------------------------------
    
test_ID = "A_BX_EmbeddedSW_KUDPCLOSE_0002"
...
try:
    ...
    print "***************************************************************************************************************"
    print "%s:To check syntax and input range for the AT command +KUDPCLOSE, Close Current UDP Operation" % test_ID
    print "***************************************************************************************************************"
    ...
    print '\nStep 4: Module A: Configure module as server and UDP socket\n'
    SagSendAT(uart_com, 'AT+KUDPCFG=,1,5043\r')
    SagWaitnMatchResp(uart_com, ['\r\n+KUDPCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(uart_com, ['OK\r\n'], 2000)
    
    print '\nStep 5: Module B: configure as client and connect to UDP server\n'
    SagSendAT(aux1_com, 'AT+KUDPCFG=,0\r')
    SagWaitnMatchResp(aux1_com, ['\r\n+KUDPCFG: 1\r\n'], 2000)
    SagWaitnMatchResp(aux1_com, ['OK\r\n'], 2000)
    
    print '\nStep 6: Module B: Send data to UDP server: \n'
    SagSendAT(aux1_com, 'AT+KUDPSND=1,%s,5043,"we are testing"\r' %uart_UDP_ip)
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    
    SagWaitnMatchResp(uart_com, ['\r\n+KUDP_DATA: 1,14,"%s",*,we are testing\r\n' %aux1_UDP_ip], 2000)
    
    print '\nStep 7: Module A: Close current UDP connection\n'
    SagSendAT(uart_com, 'AT+KUDPCLOSE=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)

    print '\nStep 8: Module A: Delete current UDP connection\n'
    SagSendAT(uart_com, 'AT+KUDPDEL=1\r')
    SagWaitnMatchResp(uart_com, ['\r\nOK\r\n'], 2000)
    
    print '\nStep 9: Module B: Close current UDP connection\n'
