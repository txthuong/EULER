...
# A_BX_EmbeddedSW_KUDP_DATA_0006                To check "+KUDP_DATA" shall not be displayed if no data is sent to the UDP socket with <data_mode>=1
#
# Requirement
# 2 Euler modules
#
...
# -----------------------------------------------------------------------------------
# A_BX_EmbeddedSW_KUDP_DATA_0006
# -----------------------------------------------------------------------------------
    
test_ID = "A_BX_EmbeddedSW_KUDP_DATA_0006"
...
try:
    ...
    SagSendAT(aux1_com, 'AT+KUDPCFG=,0,1234,1\r')
    SagWaitnMatchResp(aux1_com, ['\r\n+KUDPCFG: 1\r\n'], 3000)
    ...
    SagSendAT(aux1_com, 'AT+KUDPSND=1,"%s",1234,"A_INTEL_PROTOCOM_KUDPSND_0006"\r' %uart_UDP_ip)
    SagWaitnMatchResp(aux1_com, ['\r\nOK\r\n'], 2000)
    
    print '\nDUT: Check URC "+KUDP_DATA" is received\n'
    SagWaitnMatchResp(uart_com, ['\r\n+KUDP_DATA: 1,29,"%s",*,A_INTEL_PROTOCOM_KUDPSND_0006\r\n' %aux1_UDP_ip], 2000)
    
    print '\nAUX: Waiting about 60 seconds to check if +KUDP_DATA can be received\n'
    time.sleep(60)
    response = SagWaitResp(aux1_com, ['\r\n+KUDP_DATA: 1,29,"%s",*,A_INTEL_PROTOCOM_KUDPSND_0006\r\n' %uart_UDP_ip], 2000)
    if '+KUDP_DATA' in response:
        print ('\r\nFAIL\r\n')
        
    if '+KUDP_DATA' not in response:
        print ('\r\nOK, "+KUDP_DATA" shall not be displayed if no data is sent to the UDP socket with <data_mode>=1 \r\n')
